...
namespace Weikio.EventFramework.EventSource.Files
{
    ...
    public class FileCloudEventSource : IHostedService
    {
        ...
        private async Task ProcessFile(string sourceFilePath)
        {
            ...
            {
                var fileMoved = false;
                Exception fileMoveException = null;
                ...
                if (fileMoved == false)
                {
                    throw new Exception($"Failed to process file from path {sourceFilePath}. Couldn't move it into {processingFilePath}", fileMoveException);
                }
